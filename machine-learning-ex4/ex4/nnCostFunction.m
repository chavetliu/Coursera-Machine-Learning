...
eye_class=eye(num_labels);
for i=1:m
    y_class(i,:)=eye_class(y(i),:);
end

J = (-y_class.*log(h)-(1.0-y_class).*log(1.0-h))/m;
J = sum(sum(J));
...
L2 = ones(size(Theta2));
L2(:,1)=0;
Theta2_grad = (Delta2 + lambda*Theta2.*L2)/m;

L1 = ones(size(Theta1));
L1(:,1)=0;
Theta1_grad = (Delta1 + lambda*Theta1.*L1)/m;
